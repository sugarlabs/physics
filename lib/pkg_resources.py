...
class ZipProvider(EggProvider):
    ...
    def _extract_resource(self, manager, zip_path):
        ...
        date_time = (
            ...
            # hms, etc.
            (t & 0xFFFF) >> 11, (t >> 5) & 0x3F, (t & 0x1F) * 2, 0, 0, -1
        )
...
class EntryPoint(object):
    ...
    def require(self, env=None, installer=None):
        ...
        map(working_set.add, working_set.resolve(
            self.dist.requires(self.extras), env, installer))
...
def _sort_dists(dists):
    tmp = sorted([(dist.hashcmp, dist) for dist in dists])
...
class Requirement:
    ...
    def __init__(self, project_name, specs, extras):
        ...
        index = sorted(
            [(parse_version(v), state_machine[op], op, v) for op, v in specs])
